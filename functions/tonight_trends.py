...
def tonight_trends_fcpxml(file, library_location, event_name, path):
    ...
    bookmark_out_tonight_trends = ET.SubElement(
        media_rep_out_tonight_trends, "bookmark"
    )
    ...
    match_media2_smart_collection2_tonight_trends = ET.SubElement(
        smart_collection2_tonight_trends,
        "match-media",
        rule="is",
        type="videoWithAudio",
    )
    ...
    fcpxml_string_tonight_trends = ET.tostring(
        fcpxml_root_tonight_trends, encoding="unicode"
    )
    ...
    with open(file_path, "w") as handle:
        handle.write(fcpxml_tonight_trends)
